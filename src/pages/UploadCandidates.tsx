  ...
  const finalizeUpload = async (forceAll: boolean = false) => {
    ...
    try {
      ...
      const formattedRows = targetRows.map(({ status, errors, ...rest }) => ({
        ...rest,
        name: rest.name?.trim() ? rest.name : "NULL",
        email: rest.email?.trim() ? rest.email : "NULL",
        skills: rest.skills?.trim() ? rest.skills : "NULL",
        experience: rest.experience?.trim() ? rest.experience : "NULL",
        location: rest.location?.trim() ? rest.location : "NULL",
        batch_id: batchId,
