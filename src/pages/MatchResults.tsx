      ...
      if (allCandidates && allCandidates.length > 0) {
        ...
        for (let i = 0; i < allCandidates.length; i += chunkSize) {
          ...
          const response = await fetch("/api/match/score", {
            ...
            body: JSON.stringify({ job, candidates: chunk })
          });
          
          if (!response.ok) {
            console.error(`Matching API failed for batch ${i/chunkSize + 1}`);
            // If one batch fails, we try to continue with the others instead of completely failing
            continue;
          }
          
          const data = await response.json();
          if (data.candidates) {
            allScoredCandidates = [...allScoredCandidates, ...data.candidates];
            // Stream results immediately to UI
            setCandidates([...allScoredCandidates].sort((a, b) => (b.match_percentage || 0) - (a.match_percentage || 0)));
          }
        }
        
        // Final sort across all chunks
        allScoredCandidates.sort((a, b) => (b.match_percentage || 0) - (a.match_percentage || 0));
        setCandidates(allScoredCandidates);
      } else {
        setCandidates([]);
      }
    } catch (error) {
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {isMatching && filteredCandidates.length === 0 ? (
                          ...
                          >
                            ...
                            <td className="px-6 py-4">
                              <div className="space-y-1">
                                <p className="text-[10px] font-semibold text-slate-700">{c.strengths}</p>
                                {c.gaps && (
                                  <p className="text-[9px] text-slate-500 italic">⚠️ {c.gaps}</p>
                                )}
                              </div>
                            </td>
                            <td className="px-6 py-4 text-center">
