...
router.post('/score', async (req, res) => {
  try {
    ...
    if (candidates.length > 50) {
      return res.status(400).json({ error: "Cannot process more than 50 candidates at a time. Please reduce your candidate pool." });
    }

    // Send all candidates to AI deep dive
...
`;
    ...
    if (topCandidates.length > 0) {
      ...
      for (let i = 0; i < topCandidates.length; i += chunkSize) {
        chunks.push(topCandidates.slice(i, i + chunkSize));
      }

      const chunkPromises = chunks.map(chunk => {
        const candidatePayload = chunk.map((c: any) => ({
          id: c.id,
          name: c.name,
          skills: c.skills,
...
Evaluate the candidates and return the JSON object as requested.
      ...
      for (const response of responses) {
        ...
        const results = parsedData.scored_candidates || [];
        scoredList = [...scoredList, ...results];
      }
    ...
    // Merge AI scores for top candidates
    const finalCandidates = topCandidates.map((c: any) => {
    ...
    // Final sort
    finalCandidates.sort((a: any, b: any) => (b.match_percentage || 0) - (a.match_percentage || 0));
...
`;
    ...
    if (topJobs.length > 0) {
      const chunkSize = 10;
      const chunks = [];
      for (let i = 0; i < topJobs.length; i += chunkSize) {
        chunks.push(topJobs.slice(i, i + chunkSize));
      }

      const chunkPromises = chunks.map(chunk => {
        const jobPayload = chunk.map((j: any) => ({
          id: j.id,
          title: j.job_title,
          skills: j.skills,
...
Evaluate the jobs against this candidate and return the JSON object as requested.
      ...
      for (const response of responses) {
        ...
        const parsedData = JSON.parse(content);
        const results = parsedData.scored_jobs || [];
        scoredList = [...scoredList, ...results];
      }
    ...
    // Merge AI scores with original top job objects
    const finalJobs = topJobs.map((j: any) => {
